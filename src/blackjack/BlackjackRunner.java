		...
		while(true) {
			...
			if(res.toLowerCase().equals("hit")) {
				...
				while(yourTurn) {
					playerHand.add(deck.remove(0));
					if(p.handSum() > 21) {
						JOptionPane.showMessageDialog(null, p.getHand() + p.getSum() + "\nOh no! You busted!", "You Busted!", JOptionPane.ERROR_MESSAGE);
						continue gameloop;
					}
						...
						String hit = JOptionPane.showInputDialog(p.getHand() + p.getSum() + "\nWant to hit again?");
